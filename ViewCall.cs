...
namespace PROMPT
{
    public partial class ViewCall : Form
    {
        ...
        public ViewCall(string ProductID, String CustId, string CallID, string FormName)
        {
            InitializeComponent();
        ...
        private void frmAddCall_Load(object sender, EventArgs e)
        {
           ...
           DbCommand dbcommand = database.GetSqlStringCommand("select p.ProductName as Product,b.BrandName as Brand,cp.Capacity as Capacity" +
                                                                      ...
                                                                      "case when c.ThirdPartyID=0 then 'None' else (select ThirdPartyName from tblThirdParty where ThirdPartyID=c.ThirdPartyID) end as 'Dealer Name',c.ThirdPartyID" +
           ...
           //{
               ...
               txtThirdPartyId.Text = dtProductDetails.Rows[0]["ThirdPartyID"].ToString(); 
           //}
           //else if (dtProductDetails.Rows[0]["Status"].ToString() == "2")
           //{
           //    txtStatus.Text = "AMC";
           //}
           ...
           //}
               txtThirdparty.Text = dtProductDetails.Rows[0]["Dealer Name"].ToString().ToUpper();
           ...
           if (txtCallID.Text != "0")
           {
               ...
               dbcommand = database.GetSqlStringCommand("SELECT [CallLogId],[Calldate],[CustomerId],[ProductId],[ReasonCode],[CallDescription],[CallStatus],[Charges],[EngineerID],[EngineerDescription],[PaidAmount],CallCompleteDate FROM tblCallLogDetails where CallLogId='" + txtCallID.Text + "'");
                ...
                txtCallDate.Text = Convert.ToDateTime(callDetails.Rows[0]["Calldate"].ToString()).ToString("dd/MM/yyyy"); 
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtCallID.Text == "0")
                {
                    ...
                    dbcommand = database.GetSqlStringCommand("INSERT INTO tblCallLogDetails values (convert(Datetime,'" + txtCallDate.Text + "',103),'" + txtCustID.Text + "','" + txtProductID.Text + "','" + cmbComplaint.SelectedValue + "','" + txtDesc.Text + "','" + Status + "','" + txtCharges.Text + "','" + cmbEngineerName.SelectedValue + "','" + txtEngineerDesc.Text + "','" + txtPaidAmount.Text + "')");
                ...
                {
                     Status = cmbCallStatus.SelectedIndex == 1 ? "1" : "2";
                     dbcommand = database.GetSqlStringCommand("UPDATE tblCallLogDetails SET ReasonCode='" + cmbComplaint.SelectedValue + "',CallDescription='" + txtDesc.Text + "',CallStatus='" + Status + "',Charges='" + txtCharges.Text + "',EngineerID='" + cmbEngineerName.SelectedValue + "',EngineerDescription='" + txtEngineerDesc.Text + "',PaidAmount='" + txtPaidAmount.Text + "' WHERE CallLogId='"+txtCallID.Text+"'");
                     int result = database.ExecuteNonQuery(dbcommand);
                     if (result > 0)
                     {
